...
def main(file_to_parse):
    ...
    print(f"Total Number of messages since {datetime.datetime.fromtimestamp(all_messages[0]['timestamp_ms']/1000).strftime('%Y-%m-%d %H:%M:%S')}: {len(all_messages)}")
    for index, message in enumerate(all_messages):
        ...
        try:
            ...
            continue
    alphabetical_order = dict(sorted(all_words.items(), key=lambda item: item[0], reverse=True))
    print(json.dumps(dict(sorted(alphabetical_order.items(), key=lambda item: item[1], reverse=True)), indent=4, separators=(',', ': '), ensure_ascii=False))
